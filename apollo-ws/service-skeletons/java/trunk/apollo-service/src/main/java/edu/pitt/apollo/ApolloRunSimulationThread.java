 ...
 * Date: Apr 3, 2014
 * Time: 11:56:31 AM
 * Class: ApolloRunSimulationThread
 * IDE: NetBeans 6.9.1
 */
public class ApolloRunSimulationThread extends Thread {
    ...
    public ApolloRunSimulationThread(int runId, RunSimulationMessage message,
            ApolloDbUtils dbUtils, ApolloServiceImpl apolloServiceImpl) {
    ...
    public void run() {
        ...
        MethodCallStatusEnum status = MethodCallStatusEnum.QUEUED; // doesn't really matter
        try {
            while (status.equals(MethodCallStatusEnum.COMPLETED)) {
            ...
            ErrorUtils.writeErrorToFile("InterruptedException while attempting to get status of translator for runId " + runId
                    + ": " + ex.getMessage(),
                    apolloServiceImpl.getErrorFile(runId));
        }
        ...
        try {
            ...
            ErrorUtils.writeErrorToFile("Apollo database key not found attempting to get URL for simulator: "
                    + simulatorIdentification.getSoftwareName() + ", version: " + simulatorIdentification.getSoftwareVersion()
                    + ", developer: " + simulatorIdentification.getSoftwareDeveloper()
                    + " for run id " + runId + ": " + ex.getMessage(), apolloServiceImpl.getErrorFile(runId));
            return;
        } catch (ClassNotFoundException ex) {
            ErrorUtils.writeErrorToFile("ClassNotFoundException attempting to get URL for simulator: "
                    + simulatorIdentification.getSoftwareName() + ", version: " + simulatorIdentification.getSoftwareVersion()
                    + ", developer: " + simulatorIdentification.getSoftwareDeveloper()
                    + " for run id " + runId + ": " + ex.getMessage(), apolloServiceImpl.getErrorFile(runId));
            return;
        } catch (MalformedURLException ex) {
            ErrorUtils.writeErrorToFile("MalformedURLException attempting to create port for simulator: "
                    + simulatorIdentification.getSoftwareName() + ", version: " + simulatorIdentification.getSoftwareVersion()
                    + ", developer: " + simulatorIdentification.getSoftwareDeveloper()
                    + " for run id " + runId + ". URL was: " + url + ". Error message was: " + ex.getMessage(), apolloServiceImpl.getErrorFile(runId));
            return;
        } catch (SQLException ex) {
            ErrorUtils.writeErrorToFile("SQLException attempting to get URL for simulator: "
                    + simulatorIdentification.getSoftwareName() + ", version: " + simulatorIdentification.getSoftwareVersion()
                    + ", developer: " + simulatorIdentification.getSoftwareDeveloper()
                    + " for run id " + runId + ": " + ex.getMessage(), apolloServiceImpl.getErrorFile(runId));
            return;
        ...
        try {
            ...
            ErrorUtils.writeErrorToFile("SQLException attempting to update last service"
                    + " call for run id " + runId + ": " + ex.getMessage(), apolloServiceImpl.getErrorFile(runId));
            return;
        } catch (ClassNotFoundException ex) {
            ErrorUtils.writeErrorToFile("ClassNotFoundException attempting to update last service"
                    + " call for run id " + runId + ": " + ex.getMessage(), apolloServiceImpl.getErrorFile(runId));
            return;
        }
    }

    private static void loadTranslatorSoftwareIdentification() throws SQLException,
            ClassNotFoundException, ApolloRunSimulationException, IOException {
        ...
        for (Integer id : softwareIdMap.keySet()) {
            ...
            if (softwareId.getSoftwareName().equals("translator")) {
        ...
        if (translatorServiceRecord == null) {
            throw new ApolloRunSimulationException("Could not find translator in the list of registered services");
        }
    }
    ...
    static {
        try {
            ...
            throw new RuntimeException("IOException attempting to load the translator service record: "
                    + ex.getMessage());
//        } catch (RuntimeException ex) {
//            throw new RuntimeException("RuntimeException attempting to load the translator service record: "
//                    + ex.getMessage());
        } catch (SQLException ex) {
            throw new RuntimeException("SQLException attempting to load the translator service record: "
                    + ex.getMessage());
        }
    }
}
