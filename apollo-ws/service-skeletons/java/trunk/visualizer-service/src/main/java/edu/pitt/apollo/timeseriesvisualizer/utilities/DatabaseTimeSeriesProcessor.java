...
public class DatabaseTimeSeriesProcessor {
	...
	public void storeTimeSeriesFromDatabaseFiles(
		...
		for (String label : map.keySet()) {
			...
			String diseaseState = label.substring(0, label.indexOf(".txt"))
					.replace("_", " ");
	...
	public TimeSeriesContainer getTimeSeriesForInfectionStates(
		...
		if (getIncidenceStates) {
			if (statesToUse == null) {
				...
				statesToUse.addAll(Arrays
						.asList(InfectionStateEnum.INCIDENCE_INFECTION_STATES));
			}
		...
		for (InfectionStateEnum diseaseState : statesToUse) {
			...
			if (map.containsKey(diseaseStateValue)) {
				if (map.get(diseaseStateValue).containsKey(largestLocation)) {
					Map<Integer, Double> timeSeries = map
							.get(diseaseStateValue).get(largestLocation);
