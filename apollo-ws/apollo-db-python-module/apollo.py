...
class ApolloDB:
    ...
    def getSoftwareIdentificationForRunId(self,runId):
        ...
        if len(result) > 1:
            self.logger.update('DB_QUERY_MORE_THAN_ONE',messsage=SQLString)
            raise
        ...
        try:
            ...
            if len(result) > 1:
                raise RuntimeError("getRunDataDescription returned more than one entry "+\
                                       "for (%s,%d,%d,%s,%s)"%(label,source_software_,destination_software_,format_,type_))
    ...
    def getSimulatorIdFromRunId(self,simulationRunId_):
        try:
            ...
            result = self.query(SQLStatement)
            if len(result) > 1:
                raise RuntimeError("getSimulatorIdFromRunId: more than one run with simulationRunId %s"%str(simulationRunId))
