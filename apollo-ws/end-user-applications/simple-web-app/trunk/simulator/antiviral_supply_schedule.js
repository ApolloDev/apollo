...
var antiviralSupplySchedule = new function(){
    //private functions
    ...
    function onGridAddClicked(){
        ...
        if (currMode == 'rate'){//update the delivery rate number
            ...
            var currentCount = parseInt( $('#antiviral-supply-delivery-rate-change').val() );
            currentCount++;
            $('#antiviral-supply-delivery-rate-change').val(currentCount);
            ...
            var currentCount = parseInt( $('#antiviral-supply-delivery-number').val() );
            currentCount++;
            $('#antiviral-supply-delivery-number').val(currentCount);
        }
    ...
    function getAntiviralSchedule(){
        var mainGrid = $(dataExchange.gridId);
        var rowData = jqgridHelper.findDataByValue(mainGrid, 'pname', 'Antiviral Supply Schedule');

        if (rowData.length == 0)
            throw "Can't get Antiviral Supply Schedule";
    ...
    function initChart(rawData){		
        ...
        chart = new Highcharts.Chart({
            chart: {
                renderTo: 'antiviral-supply-delivery-chart',
    ...
    function initRateGrid(){
        ...
        //unload grid first
        deliveryGrid = $("#antiviral-supply-delivery-table");
        ...
        beforeSaveCell : function (	rowid, cellname, value, iRow, iCol){
            ...
            var dateVal = $( "#antiviral-supply-date-of-first-delivery" ).val();
...
function loadRateAry(mydata){
    ...
    $('#antiviral-supply-delivery-rate-change').val(mydata.length - 1);
		
    //hide the table control
    $('#antiviral-supply-delivery-number-div').hide();
    //show the rate control
    $('#antiviral-supply-delivery-rate-div').show();
}
...
function initTableGrid(rawData){
    ...
    //set the rate control (number of delivery)
    $('#antiviral-supply-delivery-number').val(mydata.length);
		
    //show the table control
    $('#antiviral-supply-delivery-number-div').show();
    //hide the rate control
    $('#antiviral-supply-delivery-rate-div').hide();
};
	
function initControl(){
    var beginDatePicker = $( "#antiviral-supply-date-of-first-delivery" )
    ...
    //		$('#antiviral-supply-delivery-table-add').button().click(onGridAddClicked);
    //		$('#antiviral-supply-delivery-table-insert').button().click(onGridInsertClicked);
    //		$('#antiviral-supply-delivery-table-delete').button().click(onGridDeleteClicked);
    $('#antiviral-supply-delivery-table-add').button().click();
    $('#antiviral-supply-delivery-table-insert').button().click();
    $('#antiviral-supply-delivery-table-delete').button().click();
    $('#antiviral-supply-delivery-table-add').button().hide();
    $('#antiviral-supply-delivery-table-insert').button().hide();
    $('#antiviral-supply-delivery-table-delete').button().hide();
		
    $('#antiviral-supply-schedule-submit').button().click(function (){
//        //if user is still editing the table
//        if (deliveryGrid.find('input').length != 0){
...
//            throw "Can't find Antiviral Supply Schedule Parameter!";
//			
//        rowData = rowData[0];
//			
...
//        mainGrid.setRowData(tgtRowId, rowData);
    ...
    $('#antiviral-supply-schedule-submit').button().hide();
		
    $('#antiviral-supply-rate-mode').click(onRateMode);
    $('#antiviral-supply-table-mode').click(onTableMode);
};
...
function onRateMode(){
    ...
    $('#antiviral-supply-delivery-table-insert').button( "option", "disabled", false );
};
...
function onTableMode(){
    ...
    $('#antiviral-supply-delivery-table-insert').button( "option", "disabled", true );
};
...
this.init = function() {
    try{
        ...
        var antiv = getAntiviralSchedule();
			
        //the init mode is rate grid
        //init with rate mode with the raw array data
        initRateGrid();
        loadRateRawAry(antiv);
...
var antivsupplys = antiviralSupplySchedule.init();