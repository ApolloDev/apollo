...
import edu.pitt.apollo.runmanagerservice.datastore.methods.RunJobMethod;
import edu.pitt.apollo.runmanagerservice.datastore.methods.RunJobMethodFactory;
 ...
 * @author nem41
 */
public class DatastoreAccessImpl implements SoftwareRegistryInterface, RunManagementInterface, ContentManagementInterface, UserManagementInterface, JobRunningServiceInterface {
	...
	public InsertRunResult insertRun(RunMessage message) throws RunManagementException {
		try {
			DatastoreAccessor databaseAccessor = DatastoreAccessorFactory.getDatabaseAccessor(message);
	...
	public void updateStatusOfRun(BigInteger runId, MethodCallStatusEnum statusEnumToSet, String messageToSet, Authentication authentication) throws RunManagementException {
		try {
			...
			Logger.getLogger(DatastoreAccessImpl.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
